...
for (i in 1:length(parsedList)) {
  parsedList[[i]] <- Filter(function(x) nchar(x)>=2, parsedList[[i]])
...
write.table(parsed, paste0(odir, "/", docName, "_for_cleansing.txt"), row.names=F)

# csv로 저장하기 for topic modeling
parsedDf <- data.frame(id=paste(docName, 1:length(parsed), sep="_"), parsedContent=parsed)
write.csv(parsedDf, paste0(odir, "/", docName, "_for_topic_modeling.csv"), row.names=F)
