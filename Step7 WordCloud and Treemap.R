...
wordDf <- data.frame(word=names(wordsFreq), freq=wordsFreq, row.names=NULL)
...
library(wordcloud2)
(wc2 <- wordcloud2(wordDf[-1,], size=1, fontFamily="NanumGothic", color=pal, 
                   backgroundColor="white", minRotation=-pi/4, maxRotation=pi/4, 
                   shuffle=T, rotateRatio=0.25, shape="circle", ellipticity=0.6))
...
# library(webshot)
# webshot("wc2.html", 
#         paste("./wordcloud for ", docName, ".png", sep=""), 
#         delay=5, vwidth=1080, vheight=1080)
...
# png 파일로 저장 
dir.create("./Image")
png(paste0("./Image/Tree Map for ", docName, ".png"), 
    width=1080, height=1080, pointsize=20)

library(treemap)
treemap(wordDf[-1,],                      # 대상 데이터 설정
        title="Word Tree Map",            # 제목 설정
        index=c("word"),                  # 박스 안에 들어갈 변수 설정
        vSize="freq",                     # 박스 크기 기준
        fontfamily.labels="NanumGothic",  # 폰트 설정
        fontsize.labels=14,               # 폰트 크기 설정
