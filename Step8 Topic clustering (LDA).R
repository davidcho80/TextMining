...
ggplot(data=as.data.frame(termTfIdf), aes(x=termTfIdf)) + 
  geom_histogram(breaks=seq(0,0.5,0.01), col="black", fill="white")
...
# 토픽별 핵심 단어 파일로 출력하기
(filePathName <- paste0("./output/", docName, "_", k, "_LDA_Result_terms.xlsx"))
...
# 최종 데이터프레임 생성
topicDocs <- data.frame(id=rownames(topicPr),
                        topicNo=topicNo,
...
# 문서별 토픽 번호 및 확률 저장
(filePathName <- paste0("./output/", docName, "_", k, "_LDA_Result_docs.xlsx"))
...
freqMat <- data.frame(term=colnames(dtm1Mat), 
...
jsonLDA <- createNamJson(phi=phi, 
                         theta=theta, 
                         vocab=vocab, 
                         doc.length=docLen,
                         term.frequency=freqMat$freq,
                         mds.method = canberraPCA)  # canberraPCA가 안될 때, jsPCA 할당
